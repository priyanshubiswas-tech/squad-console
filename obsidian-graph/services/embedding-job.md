...
Reads `knowledge_base/{team}/tactics_notes.md` + `public_scouting.md` for all 8 teams plus `knowledge_base/shared/tactical_theory.md`, chunks each along markdown section headers (`chunking.py`), and embeds into [[chromadb]] using Chroma's default local sentence-transformers model (`all-MiniLM-L6-v2`) — no LLM API call, no key needed.
