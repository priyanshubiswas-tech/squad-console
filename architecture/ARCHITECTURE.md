...
2. **Ingestion layer** — a Python ELT pipeline (`ingestion/elt-pipeline-py-script/deploy_elt.py`), runnable by hand on the host or scheduled via the `airflow/` DAG (`squad_console_elt`, `@daily`). Extract writes raw API responses into ClickHouse's `raw_data_store` (audit trail); Transform/mock-generate load `squad_data_store` (master); Partition fans it out into the 8 per-team databases (truncate-and-reload). Each stage is an independent function, which is what let it become Airflow tasks with no restructuring.
...
4. **RAG layer (ChromaDB + embedding job)** — built. Real, researched per-team knowledge base markdown (`tactics_notes.md`, `public_scouting.md`) plus a shared tactical-theory doc, chunked and embedded via `embedding_job/embed_team_docs.py` (host-run) into 15 Chroma collections with `{team, doc_type, visibility}` metadata, using a local sentence-transformers model - no LLM key needed for this phase. Queried live by the agent layer below on every `/api/chat` call.
5. **Agent layer (LangGraph)** — built. The real 7-node graph (`intent_router → stats_tool → rag_retriever → access_filter → reasoner → [chart_node?] → composer`) answers tactical questions using the active team's full data plus the opponent's public data only. The `access_filter` node re-applies the same `access_control.py` function the REST API already uses (see below), so there is one source of truth for what's private. Only `reasoner` needs an LLM key - it gracefully degrades to an honest fallback message when one isn't configured, rather than erroring, so the rest of the graph is verifiably exercised today. Its deterministic sibling: `backend/app/reports/generators.py` implements the same "query ClickHouse → produce `{text, chart_url}`" contract as 3 fixed-template, zero-LLM chatbot "chips" (`/api/reports/fitness`, `/top-performers`, `/financial`), calling the same `backend/app/charts/generators.py` chart functions `chart_node` calls. See the root README's "The hybrid chat design" section.
6. **Serving layer (FastAPI + Nginx)** — FastAPI exposes REST endpoints with access control: `session`, `dashboard`, `inspect`, `data-sources`, `charts`, `reports`, `chat` are all built (`../backend/`). Nginx is built and in the compose stack (`../nginx/`) as the reverse proxy, enforcing a shared `X-API-Key` secret in front of the backend - the backend independently re-checks the same key, since its port is still published directly to the host for dev convenience (see the root README's "Nginx + API key gate" section for the full reasoning). This key is a separate concern from `access_control.py` above: the key gates *access to the API at all*, the access-control matrix gates *which data a legitimate, keyed request can see*.
7. **Frontend (React + Tailwind)** — the only place the public product name/branding is meant to live. Built (`../frontend/`): login, dashboard, inspect, tactics (SVG pitch diagrams), news, and a persistent chat panel (3 chips + chatbox). Verified end-to-end in a real browser via Playwright.

## Access control (single source of truth)

`backend/app/access_control.py`, one function — `get_allowed_tables(requested_team, active_team)` — decides what's visible. Both `backend/app/routers/inspect.py` and the LangGraph `access_filter` node call it. Never duplicate this logic.
