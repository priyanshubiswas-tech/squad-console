...
6. **Serving layer (FastAPI + Nginx)** — FastAPI exposes REST endpoints with access control: `session`, `dashboard`, `inspect`, `data-sources`, `charts` are built (`../backend/`); `chat`/LangGraph comes next. Nginx isn't in the compose stack yet — frontend and backend are reached directly on their own ports.
