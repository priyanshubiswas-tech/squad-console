...
| `airflow` | `apache/airflow:2.10.4-python3.11` (custom, see `airflow/Dockerfile`) | `8080` (local-only, not routed through Nginx) | Standalone Airflow (`SequentialExecutor`) running the `squad_console_elt` DAG — see [Scheduled ingestion with Airflow](#scheduled-ingestion-with-airflow) for the login. |
...
- **Why the password needs a manual reset:** `airflow standalone` (unlike the full LocalExecutor docker-compose setup) ignores the `_AIRFLOW_WWW_USER_USERNAME`/`_AIRFLOW_WWW_USER_PASSWORD` env vars entirely and always generates its own random password on first init, writing it to `/opt/airflow/standalone_admin_password.txt` (also logged once at startup). On a fresh `airflow_home` volume, reset it to something memorable:
